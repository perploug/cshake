        ...
        function AddItemsToPackage(providerId,selectAll,items)
        {
            for (var i = 0; i < items.length; i++) { 
               ...
               if(elem == null || jQuery(elem,"#"+providerId).size() == 0)
               {
                    ...
                    var parentElem = jQuery("div [itemId = '" + parentId + "'] div.providerItems");
                    if(parentElem.length == 0)
                    {
                       parentElem = jQuery("#" + providerId + "providerItems");
                    }
                    
                    var cur = jQuery('<div/>').attr("class", "selectedItem").attr("itemId",items[i].Id).appendTo(parentElem);
